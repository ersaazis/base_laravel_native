@php
    ...
    $homeNavActive = request()->routeIs('dashboard');
    $profileNavActive = request()->routeIs('profile.*', 'settings.*', 'security.*');
@endphp
...
<section class="mobile-screen vault-pattern mx-auto grid h-[calc(100dvh-var(--inset-top,0px)-var(--inset-bottom,0px))] w-full max-w-md grid-rows-[auto_minmax(0,1fr)_auto] overflow-hidden px-5 pt-4" data-authenticated-shell data-page-shell>
    <header class="z-20 -mx-5 flex min-h-16 shrink-0 items-center justify-between gap-4 border-b border-vault-border/40 bg-vault-bg px-5 pb-3">
        <a class="flex min-w-0 items-center gap-3 rounded-2xl transition" href="{{ route('dashboard') }}" aria-label="{{ __('mobile.nav.home') }}" data-mobile-home-logo>
    ...
    <div class="min-h-0 overflow-y-auto overscroll-contain py-6" data-mobile-scroll data-page-content>
        @include('layouts.partials.feedback')

        {{ $slot }}
    </div>

    <div class="relative z-20 shrink-0" data-notification-status-url="{{ route('notifications.status') }}" data-notification-refresh="60000">
        <div class="sr-only">
            <span>{{ __('mobile.nav.home') }}</span>
            <span>{{ __('mobile.nav.profile') }}</span>
        </div>
        <nav class="-mx-5 grid grid-cols-2 gap-2 border-t border-vault-border/40 bg-vault-bg px-5 py-3" aria-label="{{ __('mobile.common.navigation') }}" data-mobile-bottom-nav>
            <a class="mobile-bottom-nav-item {{ $homeNavActive ? 'is-active' : '' }}" data-mobile-nav-item="home" href="{{ route('dashboard') }}" @if ($homeNavActive) aria-current="page" @endif>
                @include('mobile.partials.icon', ['name' => 'home', 'class' => 'size-6'])
                <span>{{ __('mobile.nav.home') }}</span>
            </a>
            <a class="mobile-bottom-nav-item {{ $profileNavActive ? 'is-active' : '' }}" data-mobile-nav-item="profile" href="{{ route('profile.edit') }}" @if ($profileNavActive) aria-current="page" @endif>
                @include('mobile.partials.icon', ['name' => 'user', 'class' => 'size-6'])
                <span>{{ __('mobile.nav.profile') }}</span>
            </a>
        </nav>
    </div>
