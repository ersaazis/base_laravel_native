<x-layouts.app :title="__('mobile.profile.title')">
    <div class="grid gap-6">
        ...
        <section class="vault-card grid gap-1 rounded-3xl" data-mobile-animate>
            ...
            <div class="mx-5 border-t border-vault-border/50"></div>
            ...
            @if ($biometricsEnabled)
                <form method="POST" action="{{ route('settings.biometrics.disable') }}" data-biometric-form>
                    ...
                    <button class="flex min-h-16 w-full items-center justify-between gap-4 px-5 py-4 text-left" type="submit">
                        <span class="flex items-center gap-3">
                            <span class="vault-icon-tile size-11 shrink-0 rounded-full">@include('mobile.partials.icon', ['name' => 'fingerprint', 'class' => 'size-6'])</span>
                            <span>
                            <span class="block font-extrabold text-vault-text">{{ __('mobile.profile.biometrics_disable') }}</span>
                            <span class="mt-1 block text-sm vault-dim">{{ __('mobile.profile.biometrics_active') }}</span>
                            </span>
                        </span>
                        <span class="vault-chip">{{ __('mobile.profile.biometrics_on') }}</span>
                    </button>
                </form>
            @else
                <form method="POST" action="{{ route('settings.biometrics.enable') }}" data-biometric-form>
                    ...
                    <button class="flex min-h-16 w-full items-center justify-between gap-4 px-5 py-4 text-left" type="submit">
                        <span class="flex items-center gap-3">
                            <span class="vault-icon-tile size-11 shrink-0 rounded-full">@include('mobile.partials.icon', ['name' => 'fingerprint', 'class' => 'size-6'])</span>
                            <span>
                            <span class="block font-extrabold text-vault-text">{{ __('mobile.profile.biometrics_enable') }}</span>
                            <span class="mt-1 block text-sm vault-dim">{{ __('mobile.profile.biometrics_subtitle') }}</span>
                            </span>
                        </span>
